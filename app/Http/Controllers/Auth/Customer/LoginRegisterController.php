...
class LoginRegisterController extends Controller
{
    ...
    public function loginRegister(LoginRegisterRequest $request)
    {
        ...
        elseif($type === 1 ){
            ...
            $emialService->setDetails($details);
            $emialService->setFrom('norply@example.com', 'example');
            $emialService->setSubject('کد احراز هویت');
            $emialService->setTo($inputs['id']);
            $messagesService = new MessageSerivce($emialService);
        }
    ...
    public function loginConfirmForm($token){
        ...
        if(empty($token)){
            return redirect()->route('auth.customer.login-confirm-form')->withErrors(['id' => 'ادرس وارد شده نامعتبر میباشد.']);
        }
    ...
    public function loginConfirm($token, LoginRegisterRequest $request)
    {
        ...
        if($otp->otp_code !== $inputs['otp']){
            ...
            return redirect()->route('auth.customer.login-confirm-form' , $token)->withErrors(['id' => 'کد وارد شده صحیح نمیباشد']);
        }
        //if everything is ok
        $otp->update(['used' => 1]);
        $user = $otp->user()->first();
    ...
    public function loginResendOtp($token)
    {
        ...
        elseif($otp->type === 1 ){
            ...
            $emialService->setFrom('norply@example.com', 'example');
            $emialService->setSubject('کد احراز هویت');
            $emialService->setTo($otp->login_id);
            $messagesService = new MessageSerivce($emialService);
        }
